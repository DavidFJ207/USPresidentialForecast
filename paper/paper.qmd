...
Additionally, the Emerson dataset included critical demographic data, voting history, and approval ratings. 
...
create_comparison_plot <- function(plot_data) {
  ggplot(plot_data, aes(
    ...
    scale_fill_manual(
      values = c("Blue State" = "blue", "Red State" = "red")
    ) +
    labs(
...
```
Beyond identifying swing states, it's also important to understand the differences between the most strongly Democratic ("blue") and Republican ("red") states. The bar graph in @fig-party-extremes takes a closer look at these differences. It focuses on key factors that show the biggest contrasts between Democratic-leaning and Republican-leaning states, helping to highlight how these factors differ between the most extreme red and blue states. To begin, we can see that for reference, we have two bars showing democratic points and republican points. You can see the questions asked by Emerson Pollster regarding who'd they vote for, and you can see that their opinion almost matches the pct given for that state.

The @fig-party-extremes visualization provides insight into how people's responses to polls may influence their political preferences.
...
Next, we take a deeper dive into the various demographic and social factors that influence voter preferences. The predictors we consider include education levels, gender, and past voting behaviour. These variables are crucial as they shape political preferences and are likely to influence which way a state leans politically.  
```{r}
...
In @fig-predictors-party-pct, we explore how these factors correlate with party lean across the U.S. states. This analysis helps us to identify which factors have the greatest influence in each state, offering a clearer understanding of what drives the electorate.  
...
```
The correlation matrix in @fig-predictors-party-pct provides a detailed look at how various predictors are related to party preferences. From this, we can see which predictors—such as education or gender—are more strongly associated with Democratic or Republican leanings. The blue shading indicates stronger correlations with Democratic support, while red represents stronger correlations with Republican support. Some key observations is that higher education tends to have a higher correlation with voting democrat and negative for Republican, and lower education supports trump more in 2020. Minority groups tend to lean democratic while causian/white tend to support trump. Previous voter behaviour also shows that biden supporters, are still democratic voters, while trump supporters remain dissaproving of biden. Lastly, there is a slight leaning towards democrats from women compared to men where they usually lean to republican.
...
time_data <- time_data %>%
  ...
  summarise(
    ...
    biden_approval = mean(`Do you approve or disapprove of the job Joe Biden is doing as President? Approve`, na.rm = TRUE)
  ) %>%
  ungroup() %>%
  group_by(party) %>%
  mutate(
  ...
  ungroup() %>%
  mutate(
    biden_approval_smooth = rollapply(biden_approval, width = 26, FUN = mean, align = "right", fill = NA)
  )
...
ggplot() +
  ...
  geom_line(data = biden_approval, aes(x = end_date, y = biden_approval_smooth, color = "Biden Approval"), size = 1) +geom_vline(xintercept = as.Date("2024-07-21"), color = "red", linetype = "dashed") +
...
```
Lastly, we look at democratic and replican points and bidens approval over time and a line at july 21st when joe biden ended @globalnews2024. It can be see this perioud, democratic support was low and increased due to this change. Using insights into how specific demographic factors, previous voting behaaviour, we will predict the 2024 election. 
...
## Model Development
The goal of our model is to forecast the likely vote outcome of 8 swing states for the 2024 US presidential election.
