...
date: today
date-format: long
abstract: This paper analyzes recent swing state polling data to forecast outcomes for the 2024 U.S. Presidential Election. Our findings indicate a close race, with Republicans leading in key states such as Georgia, Nevada, North Carolina, New Hampshire, Wisconsin, and Pennsylvania, while Democrats hold a slight edge in Michigan and Arizona. The national polling average also leans towards Republican, suggesting a Republican win in the 2024 Election. These insights give us a clearer picture of voter preferences across states and highlight the importance of targeted campaign strategies. 
format: pdf
...
The remainder of this paper is structured as follows: @sec-data details the data and measurement process; @sec-model covers the model; @sec-results provides results; and @sec-discussion discusses implications and future steps.
...
Our goal was to clean, organize, and analyze U.S. presidential election polling data to highlight voter preferences by state. We used **`knitr`** (@knitr) for report generation, ensuring our analysis was clear and well-documented, and **`kableExtra`** (@kableExtra) to enhance table outputs for easy interpretation. **`ggcorrplot`** (@ggcorrplot) helped us visualize correlations, revealing relationships between variables, while **`zoo`** (@zoo) efficiently handled time series data, such as shifts in voter preferences. The **`here`** (@here) package ensured consistent file paths, making our workflow reproducible. This streamlined approach provided a comprehensive, polished analysis of key trends and demographic distributions. @tbl-predictors-pct-summary-1 and @tbl-predictors-pct-summary-2 provide a snapshot of these findings, showcasing the results of our organized and thorough data analysis.

```{r}
#| label: tbl-predictors-pct-summary-1
#| tbl-cap: "Voter Party Preferences by State"
#| echo: false
#| warning: false
#| message: false
...
```{r}
#| label: tbl-predictors-pct-summary-2
#| tbl-cap: "Demographic Breakdowns by State"
#| echo: false
#| warning: false
#| message: false
...
## Measurement  
We chose Emerson as our primary pollster, and the reasoning for this is detailed in @sec-appendix-A. Emerson's polls offered crucial information, such as polling dates, party affiliations, sample sizes, and the percentage of support for each political party.
...
#| tbl-cap: "Final Summary Table of 95% Confidence Intervals for Common States"
#| echo: false
#| warning: false
#| message: false
...
```{r}
#| label: fig-selected-predictors
#| fig-cap: Differences in Selected Predictors Between California and South Dakota
#| echo: false
#| warning: false
#| message: false

# Update the predictor_columns to exclude gender
predictor_columns <- c(
  "What is the highest level of education you have attained? College graduate",
  "What is the highest level of education you have attained? High school or less",
  "For statistical purposes only, can you please tell me your ethnicity? White or Caucasian",
...
# Update the short_labels to exclude gender
short_labels <- c(
  "College Graduate",
  "High School or Less",
  "White or Caucasian",
...
# Prepare the poll_summary data frame with specific predictors
poll_summary <- poll_data %>%
  select(state, all_of(predictor_columns))

# Function to compute mean differences for selected predictors
compute_mean_differences <- function(data, blue_state, red_state) {
  ...
  diff_df <- tibble(
    Variable = short_labels,  # Use shortened labels
...
create_comparison_plot <- function(plot_data) {
  ggplot(plot_data, aes(
    ...
    # Customize the colors and labels for the groups
...
analysis_results <- analyze_selected_predictors(poll_summary, blue_state = "California", red_state = "South Dakota")
...
#| fig-cap: "Parties Approval and Leaning Preferences"
#| echo: false
#| warning: false
#| message: false
...
The results, summarized in Table @tbl-modelresults, show the predicted vote shares for each candidate in the swing states. Our findings show the impact of demographic and historical factors on voting behavior.

By using a Bayesian approach, we not only make point estimates but also quantify the uncertainty surrounding these predictions. This is crucial for election forecasting, where even small shifts in voter behavior can have significant implications.
...
## Predictive Insights and Electoral Dynamics
...
## The Role of Demographic Factors in Predicting Party Support
...
# Appendix: Data Cleaning {#sec-appendix-C}
...
First, we used the **`googledrive`** package (@googledrive), which allowed our code to connect to Google Sheets and download the poll data. This made it possible for us to get the most up-to-date information. Once we had the data, we used another set of packages called **`tidyverse`** (@tidyverse), which includes **`dplyr`** (@dplyr) and **`readr`** (@readr), to clean up the information and organize it. For data coming from Excel files, we used the **`readxl`** package (@readxl), which made it easy to read and work with the information from those files.

We also used **`lubridate`** (@lubridate) to make sure dates were handled correctly, so the timeline made sense and everything was in order from oldest to newest.

The **`janitor`** package (@janitor) was helpful for cleaning up column names. We also used **`testthat`** (@testthat) to test our data, making sure everything was correct and worked as expected. Finally, we used a package called **`arrow`** (@arrow) to save the data in a file format called Parquet, which makes the data smaller and faster to work with.
